...
class CodenceReview(gl.Contract):
    ...
    def analyze_and_review(
        ...
        security_prompt = (
            ...
            "Analyze this " + lang_display + " code for ALL of the following:\n\n"
            "SECURITY:\n"
            ...
            "OUTPUT FORMAT - one finding per line, strictly:\n"
            "FINDING|category|severity|line_start|title|remediation\n\n"
            "Categories: vulnerability, code_smell, performance, architecture, "
            "security_config, dependency, gas_optimization, logic_error, "
            "access_control, best_practice\n"
            "Severities: critical, high, medium, low, informational\n\n"
            "RULES:\n"
            "- Sort by severity descending (critical first)\n"
            "- Only report issues genuinely present in the code\n"
            "- Be specific: reference exact function names and variable names in titles\n"
            "- Remediation must be actionable with concrete code suggestions\n"
            "- Use pipe delimiters exactly as shown\n"
            "- No extra commentary, no markdown, no JSON wrapping\n"
            "- If no issues found, output: FINDING|best_practice|informational|0|No issues found|Code passes review\n\n"
            "CODE:\n" + code_content
        )

        equivalence_principle = (
            "Two security analyses are equivalent if they agree on the same "
            "critical and high severity findings. Specifically:\n"
            "1. Both must identify the same critical vulnerabilities (e.g., both find SQL injection)\n"
            "2. Both must agree on high severity issues (e.g., both flag resource leaks)\n"
            "3. Minor differences are acceptable: different wording, slightly different line numbers, "
            "additional low/informational findings, different ordering\n"
            "4. Category assignments for the same issue may vary slightly "
            "(e.g., vulnerability vs access_control for an auth bypass)\n"
            "5. Remediation text may differ as long as both suggest valid fixes\n"
            "6. One analysis having extra informational findings does not make them non-equivalent"
        )
